...
[→ View in onslide Studio]                  ← links to the resource (project, billing, etc.)

─────────────────────────────────
Unsubscribe from "Project shared" notifications
```

Sender: `[Tenant Name] via onslide Studio <FROM_EMAIL>`
...
| `FROM_EMAIL`          | Sender address, e.g. `noreply@onslide.io`                 |
...
- [x] Sender format: `${params.tenantName} via onslide Studio <${FROM_EMAIL}>` (email.ts line 101)
...
- [x] Fallback to "onslide Studio" if tenant lookup fails (line 74)
- **RESULT: PASS**
...
- [x] CTA button "View in onslide Studio" links to the resource (email.ts lines 58-60)
...
- [x] Fallback to `noreply@onslide.io` (email.ts line 13)
